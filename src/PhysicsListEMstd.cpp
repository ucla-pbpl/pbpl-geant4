...
void PhysicsListEMstd::ConstructProcess()
{
    ...
    G4BetheHeitler5DModel* conv = new G4BetheHeitler5DModel();
    conv->SetVerbose(5);
    gc->SetEmModel(conv);
    pm->AddDiscreteProcess(gc);
#endif

#if 0
    G4PolarizedGammaConversion* gc = new G4PolarizedGammaConversion;
    gc->SetEmModel(new G4LivermorePolarizedGammaConversionModel);
    pm->AddDiscreteProcess(gc);
#endif
    // pm->AddDiscreteProcess(new G4PolarizedGammaConversion);
